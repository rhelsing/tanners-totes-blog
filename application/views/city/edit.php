<h1 class="pad_left">Map</h1>
        ...
        <div id="pane_nav">
        	...
        	<form method="post" action="<?php echo base_url('index.php/admin/city/update/'.$city->id); ?>">
        
        <legend>City</legend>
            <div class="pure-g">
	            <div class="f_i">
		            <span class="seperate_end">
		            <input type="text" name="city" placeholder="City Name" value="<?php echo $city->city; ?>" /></span>
		         </div>
		        ...
		        foreach($states as $state){
		        	if($state->state == $city->state_id){
		        		echo '<option value="'.$state->state.'" selected="TRUE">'.$state->state.'</option>';
		        	}else{
