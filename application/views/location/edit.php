<h1 class="pad_left">Map</h1>
        ...
        <div id="pane_nav">
        	...
        	<form method="post" action="<?php echo base_url('index.php/admin/location/update/'.$location->id); ?>">
        
        <legend>City</legend>
            <div class="pure-g">
	            <div class="f_i">
		            <span class="seperate_end">
		            <input type="text" name="name" placeholder="Location Name" value="<?php echo $location->name; ?>" /></span>
		         </div>
		        ...
		        foreach($cities as $city){
		        	if($city->id == $location->city_id){
		        		echo '<option value="'.$city->id.'" selected="TRUE">'.$city->city.'</option>';
		        	}else{
